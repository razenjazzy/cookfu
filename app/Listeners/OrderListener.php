...
class OrderListener
{
    ...
    private function _onComplete()
    {
        ...
        if($adminEarningType == 'per_order') {
            ...
            $adminShareInPacket = floatval(Setting::where('key', 'admin_fee_for_order_in_packet')->first()->value);
            $storeEarnings = $this->order->subtotal - $adminShareInPacket - ($this->order->subtotal * $adminShareInOrder) / 100;
            $adminTotalEarning = $adminShareInPacket + ($this->order->subtotal * $adminShareInOrder) / 100; // admin gets it's share of earning
        }
        ...
        if($adminTotalEarning !== 0) {
            Transaction::create([
                'title' => "Admin's Commision test no",
